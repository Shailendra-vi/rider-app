...
export async function requireRider(req, res, next) {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT r.id, r.name, r.is_online, r.status,
      ...
      WHERE s.token_digest = $1 AND s.revoked_at IS NULL AND s.expires_at > now()
        ...
        AND (r.email_verified_at IS NOT NULL OR r.phone_verified_at IS NOT NULL)`,
      [tokenDigest(match[1])],
    );
...
export function requireActiveRider(req, res, next) {
  if (
    req.rider.status !== 'active' ||
    req.rider.identity_verification_status !== 'verified'
  ) {
    return next(
      new HttpError(
        403,
        'RIDER_NOT_ACTIVATED',
        'Identity verification and account activation are required for deliveries',
      ),
    );
  }
  next();
