    ...
    <section className="card">
      ...
      <div className="grid">
        ...
        <div className="box">
          <h3>Plan</h3>
          <input
            placeholder="Code e.g. WEEKDAY_LUNCH_VEG"
            value={plan.code}
          ...
          <span className="muted small">
            ₹{(plan.pricePaise / 100).toFixed(2)} — money is stored in paise, never
            decimals
          </span>
          ...
          <button
            ...
            onClick={() =>
              onAction(async () => {
                ...
                setPlan({ code: '', name: '', pricePaise: 12000 });
              })
            }
          >
            Add plan
        ...
        <div className="box wide">
          ...
          <select
            value={sub.planId}
            onChange={(e) => setSub({ ...sub, planId: e.target.value })}
          >
            ...
            {plans.map((p) => (
              <option
                ...
                value={p.id}
              >
                {p.code} — ₹{(p.price_paise / 100).toFixed(0)}
              </option>
            ))}
          </select>
          ...
          <div className="formrow">
            <label>
              ...
              <input
                type="date"
                value={sub.startDate}
                onChange={(e) => setSub({ ...sub, startDate: e.target.value })}
              />
            </label>
            <label>
              Days
              <select
                value={sub.weekdayMask}
                onChange={(e) => setSub({ ...sub, weekdayMask: Number(e.target.value) })}
              >
