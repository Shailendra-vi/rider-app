...
test('desktop scene persists across login and signup', async ({ page }, testInfo) => {
  ...
  await page.locator('canvas').evaluate((canvas) => {
    canvas.dataset.sceneInstance = 'original';
  });
...
test('mobile phone signup verifies OTP and opens the pending account', async ({
  page,
}, testInfo) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.route('**/auth/signup/start', (route) =>
    route.fulfill({
      status: 202,
      json: {
        challengeId: rider.id,
  ...
  await page.route('**/rider/me', (route) =>
    route.fulfill({ json: { rider, currentOrder: null } }),
  );
  await page.goto('/');
  await page.getByRole('button', { name: 'Join us', exact: true }).click();
  ...
  await page.screenshot({
    path: testInfo.outputPath('signup-mobile.png'),
    fullPage: true,
  });
...
test('email validation, password visibility and reset flow', async ({ page }) => {
  ...
  await page.route('**/auth/password/reset/start', (route) =>
    route.fulfill({
      status: 202,
      json: {
        challengeId: rider.id,
...
test('rate-limited OTP requests show a wait state without sending again', async ({
  page,
}) => {
  let sends = 0;
  ...
  await expect(
    page.getByRole('button', { name: 'Send login code', exact: true }),
  ).toBeDisabled();
