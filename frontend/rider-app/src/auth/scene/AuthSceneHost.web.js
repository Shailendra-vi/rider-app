...
export default function AuthSceneHost({ motionEnabled, mode }) {
  ...
  return <SceneBoundary><Canvas orthographic camera={sceneConfig.camera} dpr={[1, 1.5]} frameloop={motionEnabled ? 'always' : 'demand'}
    fallback={<AuthSceneFallback />} gl={{ antialias: true, alpha: true }}
    onCreated={({ gl }) => { gl.domElement.addEventListener('webglcontextlost', () => setLost(true), { once: true }); }}>
    <AuthScene motionEnabled={motionEnabled} mode={mode} />
