    ...
    try {
      ...
      if (typeof online !== 'boolean') {
        return res.status(400).json({
          error: { code: 'VALIDATION_FAILED', message: '"online" must be a boolean' },
        });
      }
    ...
    try {
      if (!req.rider.is_online) {
        return res.status(409).json({
          error: { code: 'RIDER_OFFLINE', message: 'Go online before claiming orders' },
        });
      }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!Array.isArray(pings)) {
        return res.status(400).json({
          error: { code: 'VALIDATION_FAILED', message: '"pings" must be an array' },
        });
      }
