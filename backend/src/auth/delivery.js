...
export async function deliverOtp({ challengeId, channel, contact, code, purpose }) {
  if (config.auth.deliveryMode === 'console' && config.env === 'development') {
    console.info(
      `[DEV OTP] ${purpose} ${channel} ${contact} challenge=${challengeId} code=${code}`,
    );
    return;
  }
  if (
    config.auth.deliveryMode !== 'webhook' ||
    !config.auth.deliveryUrl ||
    !config.auth.deliveryToken
  ) {
    throw new HttpError(
      503,
  ...
  try {
    const response = await fetch(config.auth.deliveryUrl, {
      ...
      headers: {
        ...
        Authorization: `Bearer ${config.auth.deliveryToken}`,
      },
      body: JSON.stringify({
        challengeId,
        channel,
