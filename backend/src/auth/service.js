...
const invalidCode = () =>
  new HttpError(400, 'INVALID_OTP', 'Code is invalid, expired, or already used');
const invalidLogin = () =>
  new HttpError(401, 'INVALID_CREDENTIALS', 'Unable to sign in with these credentials');
...
export async function limitAuthIp(ip) {
  await limit(`request-ip:${ip}`, 120, 3600);
}
...
async function findRider(client, channel, contact, lock = false) {
  ...
  const { rows } = await client.query(
    `SELECT * FROM riders WHERE ${column} = $1${lock ? ' FOR UPDATE' : ''}`,
    [contact],
  );
  return rows[0];
}
...
function eligible(rider, channel) {
  return (
    rider &&
    ['pending', 'active'].includes(rider.status) &&
...
export async function issueSession(client, riderId) {
  ...
  const { rows } = await client.query(
    `INSERT INTO rider_sessions (rider_id, token_digest, expires_at)
    VALUES ($1, $2, now() + interval '7 days') RETURNING expires_at`,
    [riderId, tokenDigest(token)],
  );
...
export function createAuthService({ sendOtp = deliverOtp } = {}) {
  async function createChallenge(data, purpose, ip, previousId = null) {
    ...
    const result = await transaction(async (client) => {
      ...
      if (previousId) {
        const { rows } = await client.query(
          'SELECT consumed_at FROM rider_auth_challenges WHERE id = $1 FOR UPDATE',
          [previousId],
        );
      ...
      await client.query(
        `UPDATE rider_auth_challenges SET consumed_at = now(), password_hash = NULL
        WHERE channel = $1 AND contact = $2 AND purpose = $3 AND consumed_at IS NULL`,
        [channel, contact, purpose],
      );
      const { rows } = await client.query(
        `INSERT INTO rider_auth_challenges
        ...
        [
          ...
          channel,
          contact,
          purpose !== 'signup' && canSend ? rider.id : null,
          data.name ?? null,
          canSend ? passwordHash : null,
          codeDigest(id, code),
          rider?.credential_version ?? 0,
        ],
      );
    ...
    if (result.canSend) {
      try {
        ...
        await pool.query(
          'UPDATE rider_auth_challenges SET delivered = true WHERE id = $1',
          [id],
        );
      } catch {
        await pool.query(
          `UPDATE rider_auth_challenges SET consumed_at = now(), password_hash = NULL WHERE id = $1`,
          [id],
        );
  ...
  async function verify({ challengeId, code, password }, purpose, ip) {
    ...
    const { rows: peek } = await pool.query(
      'SELECT channel, contact FROM rider_auth_challenges WHERE id = $1',
      [challengeId],
    );
    if (peek[0])
      await limit(`verify-contact:${peek[0].channel}:${peek[0].contact}`, 20, 3600);
    const newPasswordHash = purpose === 'reset' ? await hashPassword(password) : null;
    return transaction(async (client) => {
      const { rows } = await client.query(
        'SELECT * FROM rider_auth_challenges WHERE id = $1 FOR UPDATE',
        [challengeId],
      );
      ...
      if (
        ...
        return invalidCode();
      await client.query(
        'UPDATE rider_auth_challenges SET attempts = attempts + 1 WHERE id = $1',
        [challengeId],
      );
      if (!equalDigest(challenge.code_digest, codeDigest(challengeId, code)))
        return invalidCode();
      let rider;
      if (purpose === 'signup') {
        const { rows: created } = await client.query(
          ...
          [
            ...
            challenge.channel === 'phone' ? challenge.contact : null,
        ...
        if (
          !eligible(rider, challenge.channel) ||
          rider.id !== challenge.rider_id ||
          rider.credential_version !== challenge.credential_version
        )
          rider = null;
      }
      await client.query(
      ...
      if (purpose === 'reset') {
        await client.query(
          'UPDATE riders SET password_hash = $2, credential_version = credential_version + 1, updated_at = now() WHERE id = $1',
          [rider.id, newPasswordHash],
        );
        await client.query(
          'UPDATE rider_sessions SET revoked_at = now() WHERE rider_id = $1 AND revoked_at IS NULL',
          [rider.id],
        );
  ...
  return {
    start: createChallenge,
    verify,
    async resend(challengeId, ip) {
      const { rows } = await pool.query(
        `SELECT * FROM rider_auth_challenges WHERE id = $1
        AND consumed_at IS NULL AND created_at > now() - interval '1 day'`,
        [challengeId],
      );
