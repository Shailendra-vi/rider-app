...
if (!['disabled', 'console', 'webhook'].includes(deliveryMode)) throw new Error('Invalid OTP_DELIVERY_MODE');
if (env === 'production') {
  if ((process.env.OTP_SECRET || '').length < 32) throw new Error('Production requires OTP_SECRET with at least 32 characters');
  if (deliveryMode !== 'webhook') throw new Error('Production requires webhook OTP delivery');
  if (!process.env.OTP_DELIVERY_URL?.startsWith('https://') || !process.env.OTP_DELIVERY_TOKEN) {
    throw new Error('Production requires HTTPS OTP_DELIVERY_URL and OTP_DELIVERY_TOKEN');
  }
  if ((process.env.OPS_API_KEY || '').length < 32) throw new Error('Production requires OPS_API_KEY with at least 32 characters');
}
