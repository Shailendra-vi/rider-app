...
export async function applyTransition(orderId, toStatus, actor = {}) {
  ...
  try {
    ...
    const { rows } = await client.query(
      'SELECT status, rider_id FROM orders WHERE id = $1 FOR UPDATE',
      [orderId],
    );
    if (rows.length === 0) {
    ...
    if (current === toStatus && (!riderId || rows[0].rider_id === riderId)) {
      const { rows: order } = await client.query('SELECT * FROM orders WHERE id = $1', [
        orderId,
      ]);
    ...
    const { rows: claims } = await client.query(
      'SELECT id, rider_id FROM deliveries WHERE order_id = $1 AND released_at IS NULL',
      [orderId],
    );
    ...
    if (!isValidTransition(current, toStatus)) {
      throw new HttpError(
        409,
        'INVALID_TRANSITION',
        `Cannot move order from ${current} to ${toStatus}`,
        {
          current,
    ...
    if (activeClaim) {
      if (toStatus === 'OUT_FOR_DELIVERY') {
        await client.query('UPDATE deliveries SET picked_up_at = now() WHERE id = $1', [
          activeClaim.id,
        ]);
      }
