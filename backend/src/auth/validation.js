...
export function contactInput(input, signup = false) {
  ...
  if (data.channel === 'email') {
    ...
    if (signup && !data.password)
      throw new HttpError(400, 'VALIDATION_FAILED', 'Email signup requires a password');
  } else {
    data.contact = parse(
      z
        .string()
        .regex(
          /^\+[1-9]\d{7,14}$/,
          'Use a phone number with country code, e.g. +919876543210',
        ),
      data.contact,
    );
    if (data.password !== undefined)
      throw new HttpError(
        400,
        'VALIDATION_FAILED',
        'Phone signup uses OTP, without a password',
      );
  }
  return data;
}
