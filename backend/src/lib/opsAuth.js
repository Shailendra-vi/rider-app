...
export function requireOps(req, res, next) {
  ...
  if (!config.auth.opsApiKey || !key || !equalDigest(tokenDigest(key), tokenDigest(config.auth.opsApiKey))) {
    return next(new HttpError(401, 'OPS_AUTH_REQUIRED', 'Operations authentication is required'));
  }
  req.isOps = true;
